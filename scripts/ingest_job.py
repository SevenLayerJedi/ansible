...
def main():
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("""
                SELECT job_id, job_type, job_category, job_target, job_created_date
                FROM tbl_job_queue
            ...
            job = cursor.fetchone()
            if job:
                job_id, job_type, job_category, job_target, job_created_date = job
                #
                ...
                current_utc = datetime.utcnow()
                cursor.execute("""
                    INSERT INTO tbl_jobs (job_id, job_type, job_category, job_target, job_status, job_owner,
                                          job_percent_complete, job_created_date, job_start_date,
                                          job_end_date, job_date_last_updated)
                    VALUES (%s, %s, %s, %s, 'New', %s, 0, %s, %s, NULL, %s)
                """, (job_id, job_type, job_category, job_target, hostname, job_created_date, current_utc, current_utc))
                print(Fore.GREEN + "  [+] Job inserted into tbl_jobs")
                update_job_status(job_id, 'NMAP Job Created from Queue')
                #
                ...
                update_job_status(job_id, 'NMAP Job Removed from Queue')
                #
                # Commit changes
                connection.commit()
                #
